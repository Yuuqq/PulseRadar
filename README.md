...
PulseRadar 是一个可私有部署的新闻聚合引擎。它每隔固定周期从 20+ 热榜平台和 60+ RSS 源抓取数据，经过关键词匹配、频率统计和 AI 分析后，生成瀑布流可视化报告，并通过 9 种通道推送到你的终端。

```
20+ 热榜平台 ─┐                              ┌─ 瀑布流 HTML 报告
60+ RSS 源   ─┼─→ 爬取 → 筛选 → AI 分析 ──→ ├─ 9 通道推送
Extra API    ─┘                              └─ MCP 协议查询
```
...
| 能力 | 说明 |
|---|---|
| **多源采集** | 今日头条、微博、知乎、B站、36氪、澎湃、华尔街见闻、Hacker News、Product Hunt、GitHub Trending 等 20+ 平台 |
| **RSS 聚合** | 接入任意 RSS 源，支持新鲜度过滤和每源条目上限 |
| **AI 分析** | 基于 LiteLLM 统一接口，兼容 100+ AI 模型（OpenAI、Claude、Gemini、本地模型等） |
| **关键词雷达** | 频率词匹配 + 权重排序，只推你关心的内容，过滤噪音 |
| **瀑布流报告** | 多列 Masonry 卡片布局，紧凑高效，支持关键词/平台双视图切换 |
| **多通道推送** | 企业微信 · Telegram · 飞书 · 钉钉 · 邮件 · Slack · Bark · ntfy · Webhook |
| **MCP 协议** | 接入 Claude Desktop 等 AI 客户端，自然语言查询热点数据 |
| **定时调度** | Docker + Cron 一键部署，支持推送窗口和每日一次控制 |
...
# ── AI 分析 ──
ai:
  model: openai/LongCat-Flash-Lite    # LiteLLM 格式
...
> 完整配置项说明参考上游 [TrendRadar 配置文档](https://github.com/sansan0/TrendRadar)

## 技术架构

```
                    ┌──────────────────────┐
                    │    NewsAnalyzer       │  ← 薄门面 (~80行)
                    │   __main__.py         │
                    └──────┬───────┬────────┘
                           │       │
              ┌────────────▼┐  ┌───▼────────────┐
              │ CrawlCoordi-│  │ AnalysisEngine  │
              │ nator       │  │                 │
              │             │  │ · 模式策略      │
              │ · 热榜爬虫  │  │ · 分析管线      │
              │ · RSS 抓取  │  │ · AI 分析       │
              ...
              └──────┬──────┘  └───┬─────────────┘
                     │             │
                CrawlOutput   AnalysisOutput    ← 冻结 DTO
```

| 模块 | 位置 | 职责 |
|---|---|---|
| CrawlCoordinator | `trendradar/core/crawl_coordinator.py` | 编排爬虫，合并结果，返回 CrawlOutput |
| AnalysisEngine | `trendradar/core/analysis_engine.py` | 模式选择 + 统计分析 + AI + 报告生成 |
| NewsAnalyzer | `trendradar/__main__.py` | 入口门面，串联两个编排器 |
| MCP Server | `mcp_server/server.py` | FastMCP 2.0 协议，提供数据查询接口 |

## 与上游 TrendRadar 的差异

本项目基于 [TrendRadar v5.5.3](https://github.com/sansan0/TrendRadar) 深度重构：

| 改动 | 说明 |
|---|---|
| **架构重构** | NewsAnalyzer 从 835 行拆分为 CrawlCoordinator + AnalysisEngine + 80 行门面 |
| **回调消除** | 5 个核心模块的 `_fn` 回调参数全部替换为直接调用 |
| **数据契约** | CrawlOutput / AnalysisOutput / RSSOutput 冻结 DTO 作为阶段边界 |
| **瀑布流报告** | 多列 Masonry 布局替代单列线性排列 |
| **质量门控** | Ruff lint + format + pre-commit hooks 全量覆盖 |
| **品牌独立** | TrendRadar → PulseRadar |
...
<details>
<summary><strong>📖 上游 TrendRadar 原始文档（点击展开）</strong></summary>

<!-- 以下为上游 TrendRadar 的原始 README，保留作为详细配置参考 -->
