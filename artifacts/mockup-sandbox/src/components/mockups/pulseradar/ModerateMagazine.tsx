...
function EditorialSparkline({ ranks, color = "#1a1a1a", strokeWidth = 1.5, height = 40 }: { ranks: (number | null)[], color?: string, strokeWidth?: number, height?: number }) {
  const width = 200;
  ...
  if (points.length === 0) {
    return <div style={{ width, height }} className="border-t border-b border-[#e5e0d8] flex items-center justify-center text-xs text-[#a39e93]">No Data</div>;
  }
        ...
        <div className="border-b-[3px] border-ink pb-6 flex flex-col items-center justify-center relative">
          <div className="absolute left-0 top-1/2 -translate-y-1/2 hidden md:block">
            ...
            <div className="text-[11px] uppercase tracking-widest font-sans-editorial text-[#555]">
              ISSUE NO. {META.hot_news_count}
            </div>
          </div>
          
          <h1 className="text-5xl md:text-6xl font-editorial font-black tracking-tight text-center">
            脉搏雷达 <span className="text-crimson font-serif mx-2">·</span> 今日热点
          </h1>
          
          <div className="absolute right-0 top-1/2 -translate-y-1/2 hidden md:block text-right">
            <div className="text-[11px] uppercase tracking-widest font-sans-editorial text-[#555] font-semibold">
              SIGNALS: {META.total_titles}
            </div>
            <div className="text-[11px] uppercase tracking-widest font-sans-editorial text-[#555]">
              {META.mode_label}
            </div>
        ...
        <div className="magazine-grid">
          ...
          <article className="flex flex-col">
            <div className="mb-4 flex items-baseline gap-4 border-b border-hairline pb-4">
              <span className="text-crimson text-[11px] font-sans-editorial font-bold uppercase tracking-[0.2em]">Lead Story</span>
            ...
            <div className="flex gap-6 items-start mb-8">
              ...
              <div className="pt-2">
                <h2 className="text-4xl md:text-5xl font-editorial font-bold leading-[1.1] mb-3">
                  {leadKeyword.word}
                </h2>
                <div className="flex items-center gap-3">
                  <span className="text-sm font-sans-editorial font-semibold bg-ink text-[#fbf9f4] px-2 py-0.5">
                    Heat Score: {leadKeyword.count}
                  </span>
            ...
            <div className="space-y-6 flex-1">
              {leadKeyword.titles.map((title, idx) => (
                <div key={idx} className="group relative">
                  ...
                  <h3 className="text-2xl font-editorial font-semibold leading-snug mb-3">
            ...
            <div className="mt-10 pt-6 border-t border-hairline">
              ...
              <div className="h-[60px] w-full border border-hairline p-2 bg-white/50">
                {leadKeyword.titles[0] && <EditorialSparkline ranks={leadKeyword.titles[0].ranks} color="#c13a2e" height={44} strokeWidth={2} />}
              </div>
            </div>
          </article>
          ...
          <aside className="border-l border-hairline pl-0 md:pl-10 pb-8 flex flex-col">
            <div className="mb-6 flex items-baseline gap-4 border-b border-hairline pb-4">
              <span className="text-ink text-[11px] font-sans-editorial font-bold uppercase tracking-[0.2em]">Trending Index</span>
            </div>
              ...
              {secondaryKeywords.map((kw, idx) => (
                <article key={kw.word} className="relative">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="text-xl font-editorial font-bold flex items-center gap-2">
                  ...
                  <div className="space-y-4 mt-4">
                    {kw.titles.slice(0, 2).map((title, tIdx) => (
                      <div key={tIdx} className="pl-6 border-l border-hairline hover:border-ink transition-colors">
                        <a href={title.url} className="block text-base font-editorial font-medium leading-snug mb-1 hover:text-crimson transition-colors">
                          {title.title}
                        </a>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 md:gap-2">
            ...
            <div className="flex-1 flex flex-wrap justify-end gap-x-6 gap-y-2">
              {Object.entries(PLATFORM_LABELS).map(([key, label]) => {
                ...
                // Count total hits across all keywords for this platform to fake an activity bar
